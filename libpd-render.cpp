...
void Bela_userSettings(BelaInitSettings *settings)
{
	...
	settings->periodSize = 128; //MODIFICATION
}
...
void midiMessageCallback(MidiChannelMessage message, void* arg); //MODIFICATION
...
bool setup(BelaContext *context, void *userData)
{
	{ // MODIFICATIONS
		initOSC();
		int ret = aubio_pitch_setup(context->audioSampleRate);
		if (ret) return false;
	}
	// Check Pd's version
	...
	libpd_add_to_search_path("/usr/lib/pd/extra/iemlib/"); // MODIFICATION
